...
class Manager:
    def __init__(self, idb, undo_handler = None, redo_handler = None):
        ...
        self.rust_signatures_controller = SignaturesController(self.view.panel_rust_signatures,
                                                               undo_handler=self.undo_handler,
                                                               redo_handler=self.redo_handler)
        ...
        self.rust_panel = self.view.panel_rust_generator
        self.rust_info = None
    ...
    def on_rust_done(self, sig_path: pathlib.Path):
        self.view.panel_rust_generator.label.setText(f'Generated signature<br><b>{str(sig_path)}</b>')
        ...
        self.rust_panel = self.view.panel_rust_signatures

    def on_rust_error(self, error: str):
        logger.error(error)
        info = (error[:128] + '...') if len(error) > 128 else error
        self.view.panel_rust_generator.label.setText(f'Failed to create signature<br><b>{info}</b>')
        self.view.panel_rust_generator.button.setEnabled(True)
    ...
    def on_rust_generate(self):
        ...
        worker = SigmakeWorker(self.rust_info, self.flair_path)
        worker.emitter.error.connect(self.on_rust_error, Qt.QueuedConnection)
        worker.emitter.done.connect(self.on_rust_done, Qt.QueuedConnection)
        worker.emitter.message.connect(self.on_rust_message, Qt.QueuedConnection)
    ...
    def populate_model(self, directory = IDA.get_ida_sig_dir()):
        ...
        if IDA.is_rust_binary():
            ...
            self.rust_signatures_controller.populate_model(util.sig_dir())
            self.rust_panel.label.setTextFormat(Qt.RichText)
            self.rust_panel.label.setText(f'Create and apply signature for Rust compiler\n'
                                          f'version <b>{self.rust_info["version"]}</b> '
                                          f'target <b>{self.rust_info["target"].value}</b>\n')
            self.view.panel_folders.folders.model.set_enabled(self.view.panel_folders.folders.model.ITEM_RUST_GENERATOR,
                                                              True)
            self.view.panel_folders.folders.model.set_enabled(self.view.panel_folders.folders.model.ITEM_RUST_SIGNATURES,
                                                              True)
