...
def start_process(port):
    try:
        ...
        if platform.system() == "Windows":
            process = subprocess.Popen([SYS_INTERPRETER_PATH, '-m', 'feeds.core.server', str(port)], env=_sysenv, creationflags=subprocess.CREATE_NO_WINDOW)
        else:
            process = subprocess.Popen([SYS_INTERPRETER_PATH, '-m', 'feeds.core.server', str(port)], env=_sysenv)
...
class RpcPipe(QRunnable):
    ...
    def start(self):
        try:
            ...
            self.client.connect(os.path.join(self.dir, f'server.log'))
        except Exception as e:
            print(e)
        return self
